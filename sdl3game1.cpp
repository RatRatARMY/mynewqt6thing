...
int game1::launch() {
    ...
    if (not bg_img or not apple_img or not cat_img) {
        SDL_Log("Cannot load these images for some reason. Make sure these images are in the assets folder: %s", SDL_GetError());
    ...
    if (not score_font or not time_font) {
        SDL_Log("Cannot load these fonts for some reason. Make sure these fonts are in the assets folder: %s", SDL_GetError());
    ...
    int score = 0;
    int time_remaining = FPS * 60;
    int apple_x = randint(0, SCREEN_WIDTH - 124);
    int apple_y = 0;
    int apple_speed = 5;
    int cat_x = 0;
    int cat_y = SCREEN_HEIGHT - 201;
    int cat_speed = 0;
    ...
    while (running) {
        ...
        if (not game_over) {
            ...
            if (apple_y > SCREEN_HEIGHT) {
                apple_y = 0;
                apple_x = randint(0, SCREEN_WIDTH - 124);
                can_collide = true;
            }
            ...
            if (time_remaining == 0) game_over = true;
        }
        SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
        SDL_RenderClear(renderer);
        SDL_FRect bg_rect = {0, 0, SCREEN_WIDTH, SCREEN_HEIGHT};
        ...
        std::string time_str = "Time left: " + std::to_string(time_remaining / 60);
